...
const Now = document.getElementById("NowDate");
 ...
 //   ThisDates에 현재 달 엘리먼트 입력
  ...
  document.querySelector("#InCalendar").innerHTML =  LastDates.join(" ")+ ThisDates.join(" ")+NextDates.join(" ");
  //모든 엘리먼트 삽입
  let todoData = JSON.parse(x.responseText)
  for(let i = 0; i<todoData.length;i++){
    for(let j=0;j<ThisDates.length;j++){
        if(todoData[i].date ==j  && document.getElementsByClassName("thisMonthDay[j]")){  // 일정의 date와 index가 겹치는 day가 있으면 배경 색 변경
          document.querySelectorAll("#InCalendar > span.thisMonthDay")[j-1].style.backgroundColor="pink"      
      }
    }
  }
